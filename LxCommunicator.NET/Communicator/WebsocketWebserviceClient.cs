...
/// <summary>
///     Client to handle websocketWebservices to loxone miniserver. Use <see cref="WebsocketWebserviceClient" /> for
///     communicating via websocket or derive from it to create your own websocketClient.
/// </summary>
public class WebsocketWebserviceClient : WebserviceClient {
	/// <summary>
	///     List of all sent requests that wait for a response
	/// </summary>
	private readonly List<WebserviceRequest> Requests = new();
	...
	/// </summary>
	public HttpWebserviceClient HttpClient { get; }
	...
	public static async Task<WebsocketWebserviceClient> GetClient(string serial, int permissions, string deviceUuid, string deviceInfo, CancellationToken cancellationToken = default) {
		...
		return new WebsocketWebserviceClient(ipInfo.GetBaseUri(serial), ipInfo.Port, permissions, deviceUuid, deviceInfo, ipInfo.PortOpenHttps);
	}
	...
	public override async Task<WebserviceResponse> SendWebservice(WebserviceRequest request) {
		switch (request?.Encryption) {
			...
			case EncryptionType.RequestAndResponse:
				...
				WebserviceRequest encryptedRequest = new(command, EncryptionType.None);
	...
	private bool ParseEventTable(byte[] content, MessageType type) {
		List<EventState> eventStates = new();
		using (BinaryReader reader = new(new MemoryStream(content))) {
			try {
				do {
